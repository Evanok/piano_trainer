...
### Practice modes (`PracticeMode`, `WaitEngine` follow mode)

`PracticeMode` (`free` / `wait` / `drill`, in `types/practice.ts`) is what drives the cursor. `drill` is a variant of `wait`, not a peer, but all three are one enum because only one can be active and they map 1:1 onto the mode selector; `Practice.tsx` derives `trainingMode`/`freeMode` from it rather than keeping separate flags.

**Free play is not "wait mode with the gate removed"** -- that distinction is the whole design. In wait mode the score gates the player, and the cursor doubles as the scroll position. Remove only the gate and a wrong note leaves the cursor stuck where the player no longer is, so the scroll freezes mid-piece while they play on. Free mode inverts the relationship: **the score follows the player.** `WaitEngine.setMode('free')` makes `noteOn` look for the nearest event at or ahead of the cursor, within `DEFAULT_FOLLOW_WINDOW_MEASURES` (2), that contains the pitch:
- match at the cursor -> behaves exactly like wait mode (chords and tolerance unchanged).
- match further ahead -> **re-anchor** onto it, dropping any half-held chord. This is what makes it self-healing: skipped notes, abandoned chords and flubbed bars all correct themselves on the next recognised note, so playing straight through a mistake keeps the right measures on screen.
- no match -> reported as an error, but the cursor never moves and never blocks.

Three constraints that shaped this:
- **The window is measured in measures, not events.** A dense passage and a sparse one hold very different numbers of notes; what matters for re-anchoring is musical distance. It is also deliberately small -- a wide window lets a pitch that recurs later drag the cursor ahead of the player.
- **Nearest match, not best match.** The conservative choice: a recurring pitch can then never pull the cursor forward early. Consequence: the follower only ever moves forward, so it will not rewind onto a repeated earlier note.
- **Free mode advances the sheet with `PianoScore.advanceCursor(steps)`, not `next()` or `goToEventIndex()`.** `next()` paints the position it leaves green, which would falsely mark skipped notes as played; `goToEventIndex()` does an O(n) whole-piece recolour, fine for a manual jump but far too slow per keypress. `advanceCursor` steps the cursor with no colouring at all and scrolls once at the end.

Free mode shows no note colours anywhere -- no green trail, no yellow "expected", no red wrong key on the `VirtualKeyboard`, and no `ScoreHud`. Those are all affordances of a mode that scores you. Errors are still counted, but `SessionStats.practiceMode` marks the session so `End` hides grade/success/combo (nothing forced a note to be right, so the figures would be flattering nonsense). The streak still counts it: `recordPracticeDay()` fires on mount, as always.

Mobile score practice **starts** in `drill` (its compact default) via a `useState` initialiser, deliberately not an effect. An effect that re-imposed the mode whenever it changed made the mode button unusable -- leaving the drill put you straight back into it.
